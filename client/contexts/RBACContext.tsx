...
export interface AuditLog {
  ...
  action: 'role_assigned' | 'role_removed' | 'permission_granted' | 'permission_revoked' | 
          'role_created' | 'role_modified' | 'role_deleted' | 'access_granted' | 'access_denied';
...
interface RBACContextType {
  ...
  createRole: (roleData: Omit<Role, 'id' | 'createdAt' | 'updatedAt'>) => Promise<{ success: boolean; error?: string }>;
  updateRole: (roleId: string, updates: Partial<Role>) => Promise<{ success: boolean; error?: string }>;
  deleteRole: (roleId: string) => Promise<{ success: boolean; error?: string }>;
  
  // Permission Management
  getAllPermissions: () => Permission[];
  getPermissionsByRole: (roleId: string) => RolePermission[];
  addPermissionToRole: (roleId: string, permission: Permission) => Promise<{ success: boolean; error?: string }>;
  removePermissionFromRole: (roleId: string, permissionId: string) => Promise<{ success: boolean; error?: string }>;
  
  // User Role Assignment
  assignRole: (userId: string, roleId: string, expiresAt?: string) => Promise<{ success: boolean; error?: string }>;
  removeRole: (userId: string, roleId: string) => Promise<{ success: boolean; error?: string }>;
  getUserRoles: (userId: string) => Role[];
  
  // Access Control
  checkPermission: (permission: string, resource?: string, conditions?: any) => boolean;
  checkMultiplePermissions: (permissions: string[]) => { [key: string]: boolean };
  canAccessResource: (resource: string, action: string, context?: any) => boolean;
  
  // Access Requests
  accessRequests: AccessRequest[];
  requestAccess: (request: Omit<AccessRequest, 'id' | 'requestedAt' | 'status' | 'approvalWorkflow'>) => Promise<{ success: boolean; error?: string }>;
  reviewAccessRequest: (requestId: string, decision: 'approved' | 'rejected', comments?: string) => Promise<{ success: boolean; error?: string }>;
  getAccessRequestsForApproval: () => AccessRequest[];
  
  // Audit & Compliance
  auditLogs: AuditLog[];
  logAction: (action: AuditLog['action'], resourceType: string, resourceId: string, changes?: any, reason?: string) => void;
  getAuditTrail: (filters?: { userId?: string; action?: string; dateRange?: { start: string; end: string } }) => AuditLog[];
  ...
  // Bulk Operations
  bulkAssignRoles: (assignments: { userId: string; roleId: string }[]) => Promise<{ success: number; failed: number; errors: string[] }>;
  bulkRemoveRoles: (assignments: { userId: string; roleId: string }[]) => Promise<{ success: number; failed: number; errors: string[] }>;
  
  // Temporary Permissions
  grantTemporaryPermission: (userId: string, permission: Permission, expiresAt: string, reason: string) => Promise<{ success: boolean; error?: string }>;
  revokeTemporaryPermission: (userId: string, permissionId: string) => Promise<{ success: boolean; error?: string }>;
...
const mockPermissionGroups: PermissionGroup[] = [
  {
    ...
    order: 1,
    permissions: [
      { id: 'perm-dashboard-farmer', name: 'View Farmer Dashboard', nameArabic: 'عرض لوحة الفلاح', resource: 'dashboard', action: 'read' },
      { id: 'perm-dashboard-agronomist', name: 'View Agronomist Dashboard', nameArabic: 'عرض لوحة الخبير', resource: 'dashboard', action: 'read' },
      { id: 'perm-dashboard-admin', name: 'View Admin Dashboard', nameArabic: 'عرض لوحة المدير', resource: 'dashboard', action: 'read' }
    ]
  },
  {
    id: 'group-data',
    ...
    order: 2,
    permissions: [
      { id: 'perm-soil-read', name: 'View Soil Data', nameArabic: 'عرض بيانات التربة', resource: 'soil', action: 'read' },
      { id: 'perm-soil-write', name: 'Manage Soil Data', nameArabic: 'إدارة بيانات التربة', resource: 'soil', action: 'create' },
      { id: 'perm-crop-read', name: 'View Crop Data', nameArabic: 'عرض بيانات المحاصيل', resource: 'crops', action: 'read' },
      { id: 'perm-crop-write', name: 'Manage Crop Data', nameArabic: 'إدارة بيانات المحاصيل', resource: 'crops', action: 'create' }
    ]
  },
  {
    id: 'group-admin',
    ...
    order: 3,
    permissions: [
      { id: 'perm-user-manage', name: 'Manage Users', nameArabic: 'إدارة المستخدمين', resource: 'users', action: 'create' },
      { id: 'perm-role-manage', name: 'Manage Roles', nameArabic: 'إدارة الأدوار', resource: 'roles', action: 'create' },
      { id: 'perm-system-config', name: 'System Configuration', nameArabic: 'إعدادات النظام', resource: 'system', action: 'update' }
    ]
  }
];
...
const mockRoles: Role[] = [
  {
    ...
    permissions: [
      ...
      { id: 'perm-soil-read', name: 'View Soil Data', nameArabic: 'عرض بيانات التربة', resource: 'soil', action: 'read' },
      { id: 'perm-crop-read', name: 'View Crop Data', nameArabic: 'عرض بيانات المحاصيل', resource: 'crops', action: 'read' },
      { id: 'perm-crop-write', name: 'Manage Own Crops', nameArabic: 'إدارة المحاصيل الخاصة', resource: 'crops', action: 'create' }
    ],
    isSystemRole: true,
    isActive: true,
    color: '#10b981',
  ...
  },
  {
    ...
    permissions: [
      { id: 'perm-dashboard-agronomist', name: 'View Agronomist Dashboard', nameArabic: 'عرض لوحة الخبير', resource: 'dashboard', action: 'read' },
      { id: 'perm-soil-read', name: 'View All Soil Data', nameArabic: 'عرض جميع بيانات التربة', resource: 'soil', action: 'read' },
      { id: 'perm-soil-write', name: 'Manage Soil Analysis', nameArabic: 'إدارة تحليل التربة', resource: 'soil', action: 'create' },
      { id: 'perm-crop-read', name: 'View All Crop Data', nameArabic: 'عرض جميع بيانات المحاصيل', resource: 'crops', action: 'read' },
      { id: 'perm-recommendations', name: 'Provide Recommendations', nameArabic: 'تقديم التوصيات', resource: 'recommendations', action: 'create' }
    ],
    isSystemRole: true,
    isActive: true,
    color: '#3b82f6',
  ...
  },
  {
    ...
    permissions: [
      ...
      { id: 'perm-user-manage', name: 'Manage Users', nameArabic: 'إدارة المستخدمين', resource: 'users', action: 'create' },
      { id: 'perm-role-manage', name: 'Manage Roles', nameArabic: 'إدارة الأدوار', resource: 'roles', action: 'create' },
      { id: 'perm-system-config', name: 'System Configuration', nameArabic: 'إعداد��ت النظام', resource: 'system', action: 'update' }
    ],
    isSystemRole: true,
    isActive: true,
    color: '#dc2626',
  ...
  },
  {
    ...
    level: 2,
    permissions: [
      { id: 'perm-dashboard-government', name: 'View Government Dashboard', nameArabic: 'عرض لوحة الحكومة', resource: 'dashboard', action: 'read' },
      { id: 'perm-policy-manage', name: 'Manage Policies', nameArabic: 'إدارة السياسات', resource: 'policies', action: 'create' },
      { id: 'perm-compliance-monitor', name: 'Monitor Compliance', nameArabic: 'مراقبة الامتثال', resource: 'compliance', action: 'read' },
      { id: 'perm-reports-generate', name: 'Generate Reports', nameArabic: 'إنشاء التقارير', resource: 'reports', action: 'create' }
    ],
    isSystemRole: true,
    isActive: true,
    color: '#7c3aed',
...
export const RBACProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const checkPermission = (permission: string, resource?: string, conditions?: any): boolean => {
  ...
  const checkMultiplePermissions = (permissions: string[]): { [key: string]: boolean } => {
  ...
  const logAction = (
    ...
    const auditEntry: AuditLog = {
      ...
      changes,
      reason,
      severity: action.includes('delete') || action.includes('revoked') ? 'high' : 'medium',
      category: action.includes('role') || action.includes('permission') ? 'security' : 'administrative'
    };
  ...
  const updateRole = async (roleId: string, updates: Partial<Role>) => {
    try {
      ...
      logAction('role_modified', 'role', roleId, { before: existingRole, after: updates });
  ...
  const addPermissionToRole = async (roleId: string, permission: Permission) => {
    try {
      ...
      setRoles(prev => prev.map(r => 
        r.id === roleId 
          ? { ...r, permissions: [...r.permissions, permission], updatedAt: new Date().toISOString() }
          : r
      ));
  ...
  const removePermissionFromRole = async (roleId: string, permissionId: string) => {
    try {
      ...
      setRoles(prev => prev.map(r => 
        r.id === roleId 
          ? { ...r, permissions: r.permissions.filter(p => p.id !== permissionId), updatedAt: new Date().toISOString() }
          : r
      ));
  ...
  const requestAccess = async (request: Omit<AccessRequest, 'id' | 'requestedAt' | 'status' | 'approvalWorkflow'>) => {
    try {
  ...
  const reviewAccessRequest = async (requestId: string, decision: 'approved' | 'rejected', comments?: string) => {
    try {
      ...
      logAction('access_granted', 'access_request', requestId, { decision, comments });
  ...
  const getAuditTrail = (filters?: { userId?: string; action?: string; dateRange?: { start: string; end: string } }): AuditLog[] => {
    let filteredLogs = auditLogs;

    if (filters?.userId) {
      filteredLogs = filteredLogs.filter(log => log.performedBy === filters.userId);
    }
    ...
    if (filters?.dateRange) {
      ...
      filteredLogs = filteredLogs.filter(log => 
        log.performedAt >= start && log.performedAt <= end
      );
    }
  ...
  const getRoleHierarchy = (): { role: Role; children: Role[] }[] => {
    return roles
      ...
      .map(role => ({
        role,
        children: roles.filter(r => r.level > role.level && r.level <= role.level + 1)
      }));
  };
  ...
  const bulkAssignRoles = async (assignments: { userId: string; roleId: string }[]) => {
    let success = 0;
    let failed = 0;
    const errors: string[] = [];
  ...
  const bulkRemoveRoles = async (assignments: { userId: string; roleId: string }[]) => {
    let success = 0;
    let failed = 0;
    const errors: string[] = [];
  ...
  const grantTemporaryPermission = async (userId: string, permission: Permission, expiresAt: string, reason: string) => {
    try {
      logAction('permission_granted', 'user', userId, { permission, expiresAt, reason, temporary: true });
      return { success: true };
    } catch (error) {
  ...
  const revokeTemporaryPermission = async (userId: string, permissionId: string) => {
    try {
      logAction('permission_revoked', 'user', userId, { permissionId, temporary: true });
      return { success: true };
    } catch (error) {
  ...
  const getRoleStatistics = () => {
    ...
    const mostUsedRoles = roles.map(role => ({
      role,
      count: Math.floor(Math.random() * 50) + 1
    })).sort((a, b) => b.count - a.count).slice(0, 5);

    const allPermissions = getAllPermissions();
    const permissionUsage = allPermissions.map(permission => ({
      permission,
